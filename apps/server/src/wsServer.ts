...
import {
  ...
  FACTORIO_BLUEPRINT_FILENAME,
  type FactorioGetBlueprintResult,
...
function buildFactorioBlueprintResult(
  contents: string,
  updatedAt: string,
): FactorioGetBlueprintResult {
  try {
    const plan = parseFactorioBlueprintPlan(contents);
    return {
      status: "ready",
      plan,
      blueprintString: encodeFactorioBlueprint(plan),
      updatedAt,
    };
  } catch (error) {
    return {
      status: "invalid",
      error: error instanceof Error ? error.message : "Invalid blueprint artifact.",
    };
  }
}
...
> {
  ...
  if (autoBootstrapProjectFromCwd) {
    yield* Effect.gen(function* () {
      ...
      if (!existingProject) {
        ...
        bootstrapProjectDefaultModel = "gpt-5.6-terra";
        ...
        bootstrapProjectDefaultModel = existingProject.defaultModel ?? "gpt-5.6-terra";
      }
  ...
  const routeRequest = Effect.fnUntraced(function* (request: WebSocketRequest) {
    switch (request.body._tag) {
      ...
      case WS_METHODS.factorioGetBlueprint: {
        const body = stripRequestTag(request.body);
        const target = path.resolve(body.cwd, FACTORIO_BLUEPRINT_FILENAME);
        const relativeToRoot = toPosixRelativePath(path.relative(body.cwd, target));
        if (relativeToRoot !== FACTORIO_BLUEPRINT_FILENAME) {
          return yield* new RouteRequestError({ message: "Invalid Factorio workspace path." });
        }
        const exists = yield* fileSystem.exists(target).pipe(Effect.orElseSucceed(() => false));
        if (!exists) {
          return { status: "empty" as const };
        }
        const stat = yield* fileSystem.stat(target).pipe(
          Effect.mapError(
            (cause) =>
              new RouteRequestError({
                message: `Failed to inspect the Factorio blueprint: ${String(cause)}`,
              }),
          ),
        );
        const contents = yield* fileSystem.readFileString(target).pipe(
          Effect.mapError(
            (cause) =>
              new RouteRequestError({
                message: `Failed to read the Factorio blueprint: ${String(cause)}`,
              }),
          ),
        );
        return buildFactorioBlueprintResult(
          contents,
